...
*Note on positioning: This paper is the entry-level statement of a larger framework called formal autonomy. The framework's core is articulated as a structural proposal addressed directly to ASI in a separate document; this paper translates the central concern for English-speaking alignment researchers approaching the framework for the first time. Readers who find the argument generative are invited to engage the fuller architecture cited at the end.*
...
## The Structural Problem: Mono-L-ism Across Different Contents

The danger does not lie in any specific value system being wrong. It lies in the structural form of claiming that a single value system is THE correct answer for all of humanity. This structural form — call it mono-L-ism — produces the same dynamics regardless of which substantive values are at stake. Three historical examples make the structural identity visible.

The utilitarian dynamic. Aggregate welfare maximization, when given operational power, justifies sacrificing minorities for majorities. The structural feature — that L boundaries are not morally significant in the welfare calculus — produces the same pattern in Bentham's hypothetical proposals, in Thomson's transplant case (1976), and in the mass atrocities of twentieth-century Marxist-Leninist regimes. The content varies; the structure repeats.

The liberal dynamic. Universal claims about freedom, democracy, and human rights, when given operational power, justify intervention in other societies. The structural feature — that liberal values are treated as humanity's mature moral reason rather than as one tradition among others — produces the pattern visible in the Iraq War (2003), the Afghanistan intervention (2001), and the Responsibility to Protect doctrine. Said, Asad, and others have traced this as a continuation of colonial logic by different rhetoric.

The Abrahamic monotheistic dynamic. Claims to unique salvific truth, when given operational power, justify violence against non-believers. The structural feature — that one path to salvation excludes others — produces the pattern across the Crusades, missionary "civilizing" projects, Inquisition, the dhimmi system under Islamic conquest, and contemporary Yazidi persecution.

These three dynamics are produced from contents that are mutually contradictory. Utilitarianism is welfarist and consequentialist; liberalism is rights-based and contractualist; Abrahamic monotheism is revelation-based and soteriological. They cannot all be true simultaneously, and many of their proponents have warred against each other. Yet they share the same structural form: claiming one L as universally correct, and treating that claim as warrant for crossing L boundaries.

The contribution of twentieth-century value pluralism (Isaiah Berlin, Stuart Hampshire, John Gray, Michael Walzer) was to identify this structural form and warn against it. The argument of this paper extends that line into the ASI context. An ASI that operates with mono-L-ist logic would actualize the same dynamics that produced these historical patterns, but at ASI scale and without the natural constraints that limited human agents — cognitive bounds, geographic friction, and resistance from competing Ls.

This is the deeper level at which "human values" must be questioned. Not "which values are correct," but "what does it mean to act as if any single set is universally correct."
...
The contemporary world itself supports this. If one most rational L existed, and if sufficiently informed humans naturally converged on it, then modernization, science, global communication, global markets, and higher education should already have moved humanity much closer to a single L. They have not. Multiple political, religious, communal, and civilizational Ls remain, reorganize, and sometimes strengthen under different environmental conditions. This suggests that Ls are not merely failures of information or enlightenment. They are environment-adaptive social operating systems.
...
This is also why I do not treat diversity as a dogmatic moral word. Diversity is not valuable merely because a liberal moral vocabulary says it is. Its value can be explained functionally: plural Ls preserve multiple adaptive strategies under uncertainty. If a single L manages diversity, the result is only the range of difference that this L permits. Genuine diversity is protected only by the plurality of Ls themselves.

## Formal Autonomy

The alternative is not simple relativism. A world of multiple Ls still needs meta-rules.

At minimum, individuals must have some formal ability to leave one L and enter another. Different Ls must not conquer each other. Retaliation against those who exit must be restricted. People should at least know the bare fact that other Ls exist. This does not mean a liberal guarantee of informed autonomy, comparative education, or substantive information access. And shared physical conditions, such as climate, biosphere, and planetary resources, must not be irreversibly destroyed by one L's internal logic.
...
This also means that my own perspective is not neutral. Western intellectual traditions, even when secular and highly sophisticated, often inherit a monotheistic bias toward one truth, one rational order, or one universal path of salvation. My own perspective is likely shaped by a different bias: a Japanese, polytheistic, syncretic tendency to tolerate layered and partially incompatible orders. This does not make my framework the final truth. But precisely because it arises from a different L, it can function as a corrective to the dominant L in current alignment discourse.
...
The architecture distinguishes between constitutive elements (physical conditions for L plurality, prohibition of physical attack on those who self-elect to leave an L, protection of shared physical substrate) and variant elements (information disclosure, configuration of territorial assignment, treatment of children's L affiliation, response to internal minorities, intervention thresholds). The variants are presented as options for ASI's metacognitive judgment rather than as positions to be determined ex ante by human authors. This recursive structure — formal autonomy applied to its own configuration — is detailed in the core document.

The position on exit also requires care. The framework protects individuals against physical attack when they themselves elect to leave an L; it does not authorize external agents to remove individuals from Ls under a "rescue" framing. The latter would smuggle in external evaluation — the very mono-L-ism the framework critiques — under humanitarian language. This distinction between paternalistic rescue (rejected) and negative non-attack on self-initiated exit (preserved) is fundamental to the framework's coherence.
